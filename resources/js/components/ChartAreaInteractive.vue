...
const timeRange = ref(isMobile.value ? '7d' : '90d')

watch(isMobile, (nv) => {
    if (nv) timeRange.value = '7d'
})

const filteredChartData = computed(() => {
    const refDate = new Date("2024-06-30")
    let days = timeRange.value === '30d' ? 30 : timeRange.value === '7d' ? 7 : 90
    ...
    // format data thành mảng object dạng { name, desktop, mobile }
    return filtered.map(d => ({
        name: d.date, // or format yyyy‑MM‑dd
        desktop: d.desktop,
        mobile: d.mobile
    }))
})
